            ...
            class JsonResponse:
                def __init__(self):
                    ...
                    self.url1 =\
                        ...
                        f'?input=openClassrooms&inputtype=testquery&key={self.key_map}'

                    self.url2 = 'https://maps.googleapis.com/maps/api/place/details/json'\
                                f'?placeid=ChIJIZX8lhRu5kcRGwYk8Ce3Vc8&fields=formatted_address,geometry&key={self.key_map}'
            
                    self.url3 = 'https://maps.googleapis.com/maps/api/staticmap/'\
                                'center=10 Quai de la Charente, 75019 Paris, France'\
                                '/zoom=18.5/size=600x300/maptype=roadmap/markers=color=red|label=A|48.8975156,2.3833993'\
                                f'/key={self.key_static_map}'
                def json(self):
                    if url == self.url1:
                        return result1
                    elif url == self.url2:
